          ...
          <div className="flex flex-col sm:flex-row sm:items-end gap-4 justify-between">
            <div>
              <div className="flex items-center gap-3 mb-2">
                <div className="w-10 h-10 rounded-xl bg-[#504945]/50 border border-[#665c54]/60 flex items-center justify-center">
                  <BookOpen className="w-5 h-5 text-[#a89984]" />
                </div>
                <h1 className="text-3xl font-extrabold text-[#fbf1c7]">My Bird Journal</h1>
              </div>
              <p className="text-[#928374] text-sm pl-[52px]">
                {entries.length === 0
            ...
            {entries.length > 0 && (
              <div className="relative">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#7c6f64]" />
                <input
                  type="text"
                  placeholder="Search birds..."
                  value={search}
                  onChange={e => setSearch(e.target.value)}
                  className="pl-9 pr-4 py-2 bg-[#3c3836]/80 border border-[#504945]/60 rounded-lg text-[#d5c4a1] text-sm placeholder-[#665c54] focus:outline-none focus:border-[#fabd2f] focus:ring-1 focus:ring-[#fabd2f] w-56"
                />
              </div>
            )}
          </div>
      ...
      <section className="max-w-6xl mx-auto px-4 pb-16">
        {entries.length === 0 ? (
          ...
          <div className="text-center py-20">
            <div className="w-24 h-24 rounded-full bg-[#504945]/40 border border-[#504945]/50 flex items-center justify-center mx-auto mb-6">
              <Bird className="w-12 h-12 text-[#665c54]" />
            </div>
            <h2 className="text-xl font-bold text-[#d5c4a1] mb-2">Your journal is empty</h2>
            <p className="text-[#7c6f64] mb-8 max-w-sm mx-auto">
              Go identify some birds! Record audio or upload a file on the Identify page.
            </p>
            <Link href="/">
              <Button className="bg-[#b8bb26] hover:bg-[#98971a] text-[#282828] gap-2 rounded-full">
                Start Identifying
          ...
          <div className="text-center py-16">
            <p className="text-[#7c6f64] text-lg">No birds match &quot;{search}&quot;</p>
          </div>
        ) : (
          <>
            {/* Stats bar */}
            <div className="flex flex-wrap gap-4 mb-6 text-sm text-[#928374]">
